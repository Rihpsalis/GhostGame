...
public class Player {
    ...
    public Player(boolean[] directions, double size, double screenHeight, double screenWidth) {
        ...
        for (int i = 0; i < 4; i++)
            movingImagesR[i] = new Image(new File("main/resources/player/MovingRight_" + i + ".png").toString(), playerSize, playerSize, false, false);
        movingCurImageR = movingImagesR[0];

        movingImagesL = new Image[4];
        for (int i = 0; i < 4; i++)
            movingImagesL[i] = new Image(new File("main/resources/player/MovingLeft_" + i + ".png").toString(), playerSize, playerSize, false, false);
        movingCurImageL = movingImagesL[0];

        movingImagesUp = new Image[4];
        for (int i = 0; i < 4; i++)
            movingImagesUp[i] = new Image(new File("main/resources/player/MovingUp_" + i + ".png").toString(), playerSize, playerSize, false, false);
        movingCurImageUp = movingImagesUp[0];

        movingImagesDown = new Image[4];
        for (int i = 0; i < 4; i++)
            movingImagesDown[i] = new Image(new File("main/resources/player/MovingDown_" + i + ".png").toString(), playerSize, playerSize, false, false);
